...
public class AnimateTiledTexture : MonoBehaviour
{
	...
	public float triggerAnimation=5f;
	...
	public void Update(){
		triggerAnimation-=Time.deltaTime;
		if(triggerAnimation<=0){
			triggerAnimation=Random.Range(3,6);
			// If the animation is playing, stop it
			if (_isPlaying)
			{
				StopCoroutine("updateTiling");
	...
	private void Awake()
	{
		...
		ChangeMaterial(GetComponent<Renderer>().sharedMaterials[1], _newMaterialInstance);
	}
	...
	private void ApplyOffset()
	{
		...
		// Update the material
		GetComponent<Renderer>().sharedMaterials[1].SetTextureOffset("_MainTex", offset);
